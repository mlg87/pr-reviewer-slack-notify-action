...
export const assignCodeownersAsReviewers = async (
  ...
  try {
    ...
    if (filteredUsers.length !== allUsers.length) {
      logger.info(`Filtered out PR author (${pull_request.user.login}) from reviewers list`);
    }
    ...
    try {
      ...
      assignedUsers = result.requested_reviewers?.map((user: any) => user.login) || [];
      assignedTeams = result.requested_teams?.map((team: any) => team.slug) || [];
      ...
      try {
        ...
        assignedUsers = currentReviewers.users?.map((user: any) => user.login) || [];
        assignedTeams = currentReviewers.teams?.map((team: any) => team.slug) || [];
      } catch (listError: any) {
    ...
    try {
      ...
      const autoAssignmentCommentExists = existingComments.some(
        (comment) =>
          comment.body?.includes("🤖 **Auto-assigned reviewers from CODEOWNERS**") ||
          comment.body?.includes("🤖 **Attempted to auto-assign reviewers from CODEOWNERS**") ||
          comment.body?.includes("🤖 **Processed CODEOWNERS**")
      );

      if (autoAssignmentCommentExists) {
        logger.info("Auto-assignment comment already exists, skipping duplicate");
      } else {
    ...
    logger.error(`Unexpected error in assignCodeownersAsReviewers: ${error.message}`);
    return {
      success: false,
      assigned: { users: [], teams: [] },
