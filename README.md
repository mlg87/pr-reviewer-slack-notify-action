...
Starting with v9.2.0, this action automatically assigns reviewers based on your repository's `CODEOWNERS` file before sending notifications. This eliminates the need for separate reviewer assignment workflows.
...
7. Then proceeds with the normal polling and Slack notification flow
...
sequenceDiagram
    ...
    participant Checks as CI/CD Checks
    ...
    Action->>GH: Mark as POLLING state
    Action->>Checks: Check required CI/CD status
    
    loop Poll every 90 seconds (default)
        Checks-->>Action: Checks still running
        Action->>Action: Wait polling interval
        Action->>Checks: Re-check required CI/CD status
    end
    
    Checks-->>Action: All required checks passed ✅
    Action->>Slack: Post message @mentioning reviewers
    Slack-->>Dev: Reviewers notified
    
    Action->>GH: Mark as NOTIFIED state
    Action->>GH: Add workflow summary
...
1. User opens PR (reviewers can be assigned manually or will be auto-assigned from CODEOWNERS).
1. Action automatically assigns code owners as reviewers (if CODEOWNERS file exists) and adds a comment to the PR.
1. Action begins polling for required checks to pass (every 90 seconds).
